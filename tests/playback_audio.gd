...
func run(target) -> void:
	...
	mark("divine-hand")
	await fixture(1,2)
	seed_board([[6,6],[10,5],[2,3]],1);seed_board([[7,6],[3,4]],2)
	mark("divine-hand-performance")
	var token:Vector2=game.scenery.camera.unproject_position(game.HAND_TOKEN_POSITION)
	button(token,true);button(token,false)
	await idle()
	check(game.rules.finish_kind=="divine_hand" and game.rules.wins==[1,0],"second-round pointing hand has its own complete soundtrack")
