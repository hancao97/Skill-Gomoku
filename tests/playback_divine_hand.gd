...
func run(target) -> void:
	...
	output=ProjectSettings.globalize_path("res://verification/v"+str(ProjectSettings.get_setting("application/config/version")))
	...
	await fixture(2,1)
	check(not game.hand_token.visible,"The Black third round hides the White token")
	await fixture(3,1)
	game.rules.wins.assign([2,1])
	game._refresh()
	check(game.hand_token.visible,"Fourth round restores the white stone beside the black bowl")
	await tap_token()
	check(game.rules.active() and game.rules.move_count==0,"Fourth-round opponent Black cannot activate the token")
	await click(Vector2i(14,14))
	var old_fourth:Dictionary=game.rules.serialize().duplicate(true)
	old_fourth.erase("divine_hand_used")
	game.rules.reset_match()
	check(game.rules.restore(old_fourth),"An existing fourth-round save restores successfully")
	game._start_board();await pause(1.65)
	check(game.hand_token.visible and game.rules.can_divine_hand(),"Restored fourth-round White immediately has the hand available")
	# A normal advantage White move can leave thunder ringing into the next turn.
	seed_board([[6,6],[2,3]],1)
	seed_board([[7,6]],2)
	await click(Vector2i(5,10));await idle()
	await click(Vector2i(13,13));await idle()
	check(game.rules.can_divine_hand(),"Fourth-round White can use the token after both ordinary moves")
	await capture("hand-08-fourth-round-token")
	await tap_token()
	check(game.busy and game.in_cinema and is_instance_valid(game.hand_effect),"Fourth-round real click starts the same hand performance")
	await hand_time(2.54);await capture("hand-09-fourth-round-pointing")
	await idle()
	check(area.all(func(p:Vector2i):return game.rules.at(p)==2 and game.stones.has(p) and game.stones[p].get_meta("color")==2),"Fourth-round hand lands all 65 white intersections")
	check(game.rules.at(Vector2i(14,14))==1 and game.rules.at(Vector2i(2,3))==1,"Fourth-round hand preserves stones outside its footprint")
	check(game.rules.wins==[3,1] and game.rules.match_winner==0 and game.rules.finish_kind=="divine_hand","Fourth-round hand directly awards the deciding victory once")
	await capture("hand-10-fourth-round-result")
