...
当前版本 **2.5.2**。运行与导出已在 **macOS / Apple Silicon、Godot 4.7.2、Forward+ / Metal** 上验证；其他平台尚未验证。两位玩家共用一台电脑和鼠标。
...
实机演出：[墨黑挽弓](docs/images/bow.png) · [银白月轮](docs/images/moon.png)。
...
`--hover-check` 专门检查预览中心对齐、即时跟随、落点一致及隐藏条件，结果写入包含完整补丁版本号的目录（当前为 `verification/v2.5.2/`）。
