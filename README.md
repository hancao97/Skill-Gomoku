...
当前版本 **2.4.0**。运行与导出已在 **macOS / Apple Silicon、Godot 4.7.2、Forward+ / Metal** 上验证；其他平台尚未验证。两位玩家共用一台电脑和鼠标。
...
天地大同蓄力过程中，黑子逐渐升起，周围的放射光束持续延伸、增亮；蓄满后保持待落状态，松手才发动。拖走、切换窗口或打开设置会取消手势与光束。
...
规则、设置、悬浮与震子反馈、技能互斥均有可复现检查。反馈回放还采样实际画面，确认蓄力光束逐级变亮，并覆盖长按超过音效时长后的取消与释放。当前检查数量、结果和声音测量范围见 [验证记录](VERIFICATION.md)；混音设计见 [AUDIO.md](AUDIO.md)。
