...
当前版本 **2.8.1**。运行与导出已在 **macOS / Apple Silicon、Godot 4.7.2、Forward+ / Metal** 上验证；其他平台尚未验证。两位玩家共用一台电脑和鼠标。
...
| 先手特技 | 控制隐藏技法能否触发。关闭时按普通五子棋规则落子，可独立保留优势方的落子特效。 |

「特效」和「先手特技」默认开启。关闭开关会清理相关演出或取消当前技能手势，不会回滚棋子与比分。

![特效与先手特技独立开关](docs/images/settings.png)
...
2.8.1 修复第四局缺少「神之一手」入口的问题，设置中的「特技」更名为「先手特技」。
...
| 神之一手 | **优势方执白的对局（第二、第四局）**，黑棋盒旁出现一颗白子。轮到优势白方时，点击并松开这颗白子。 | 白子聚成指向左下的大手，食指压落；**手形内的黑子和空位全部变白**，范围外保持原样，直接赢本局。 |
...
「神之一手」的触发白子保持普通外观，不加高亮或技能提示。黑方不能触发；拖开、暂停或失焦会取消点击。关闭「先手特技」会隐藏棋盒旁的白子；仅关闭「特效」时仍可发动，直接填成相同白子手形。第二局用过后，第四局可再次使用；第一、三、五局优势方执黑时隐藏入口。旧存档可继续使用，恢复第四局时会按当前棋色显示白子。
...
`--hover-check` 检查预览中心对齐、即时跟随、实际点击前后轮廓中心一致及隐藏条件，覆盖黑白棋、两种窗口尺寸和棋盘中央与四角，结果写入包含完整补丁版本号的目录（当前为 `verification/v2.8.1/`）。

`tests/test_hand_gesture.gd` 检查 145 个插值姿态的完整轮廓、指头连接、65 次落印及跨帧事件顺序。

`--hand-check` 验证第二、第四局棋盒旁白子的真实鼠标点击、伸指与回腕、轮廓、指尖触盘、手形落盘、双方归属、取消与设置开关，截图和结果写入 `verification/v2.8.1/`。附加 `--hand-showcase` 可只回放这一招，适用于录制演示。

设置回放同样使用完整版本目录（当前为 `verification/v2.8.1/`），保留旧版截图与测试结果。
...
正常更新保留棋局和偏好。旧版配置缺少新增开关时，特效、先手特技默认开启。设置改名保留原有保存键，不会重置已有开关。
